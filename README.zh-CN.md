...
`vldb-lancedb` 和 `vldb-sqlite` 现在都由各自子项目维护独立文档与发布流程，而 `vldb-manager` 继续作为本地运维入口。
...
- `vldb-lancedb` 和 `vldb-sqlite` 都可以完成 `cargo build` 和 `cargo build --release`
- `vldb-manager` 已作为 Rust 2024 + `ratatui` 终端界面加入工作区
- 两个网关服务都提供 Docker 打包与部署方式
- 服务发布流程已经下沉到各自子项目维护
...
- `vldb-sqlite` 同时支持 `QueryJson` 和 `QueryStream`
- `vldb-sqlite` 的配置与运行时行为是按 SQLite 的 PRAGMA、WAL、锁和动态类型特性实现的
